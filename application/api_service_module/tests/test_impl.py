...
class TestDevModeAuthAutoDisable(unittest.TestCase):
    ...
    def test_dev_mode_with_placeholder_keys_disables_auth(self):
        ...
        # 用 mock 隔离环境: 不能直接构造 ApiService —— 本机若有 .env (dev 常见),
        # config 会把 ${API_KEY_1} 解析成真实 sentinel 值, 测试就脆弱地失败 (依赖
        # "无 .env"). 跟 test_dev_mode_with_real_keys_keeps_auth_on 一样 mock 索引.
        with patch.object(
            ApiService, "_build_key_to_tenant_index",
            return_value={"${API_KEY_1}": "default"},
        ):
            svc = ApiService(handler=MockHandler())
        # yaml 默认 auth_enabled=true + auth_type=apikey + 全占位符 key -> 启动期自动关 auth
