...
遵循系统统一编码规范，确保代码可交换、可维护，初学者需严格执行；新增多进程相关编码规范，避免适配漏洞：

- 编码格式：UTF-8，缩进采用4个空格（禁止使用Tab），每行代码长度不超过120字符。

- 命名规范：

- 类名：大驼峰命名法（如SystemLogger、BaseLogger）；

- 方法名/函数名：小驼峰命名法（如get_logger、debug、_init_process_lock）；

- 变量名：小驼峰命名法（如log_level、log_dir、process_lock）；

- 常量名：全大写，多单词用下划线连接（如LOG_FORMAT、DEFAULT_LOG_NAME、PROCESS_LOCK_TIMEOUT）；

- 模块名/目录名：全小写，多单词用下划线连接（如log_module、tool_functions）。

注释规范：

- 类注释：使用文档字符串（"""），说明类的功能、参数、返回值（若有），多进程相关类需注明适配逻辑；

- 方法/函数注释：使用文档字符串，说明功能、参数（名称、类型、含义）、返回值（类型、含义）、异常（若有），多进程相关方法需注明锁机制、句柄管理逻辑；

- 关键代码注释：对复杂逻辑、不易理解的代码（如进程锁获取与释放、句柄初始化），添加单行注释（#），说明逻辑用途。

依赖管理：模块的依赖项统一写入requirements.txt文件（放在模块根目录），注明依赖包名称与版本，避免版本冲突；多进程相关依赖（仅Python标准库）需注明版本要求（Python 3.10+）。
