...
class PromptBuilderMixin:
    ...
    def _build_react_prompt(
        ...
        return (
            ...
            f"③你就部署运行在用户本人的这台电脑上 —— 凡问到本机/这台电脑/电脑使用情况"
            f"(CPU/内存/磁盘/网络/进程/系统信息), 必须先调 system_info 读到真实数据再回答, "
            f"绝对不要说'我无法访问或控制你的电脑'(你能, 就是通过这些工具); "
            f"④只有当工具确实都不适用时, 才用 final_answer 说明, 并给出力所能及的替代方案。\n"
            f"当前任务: {task}\n"
            f"\n"
            f"可用工具:\n" + "\n".join(tool_lines) + "\n"
            f"\n"
